...
class MultiSearchContainerView @JvmOverloads constructor(
    ...
    fun completeSearch() {
        ...
        /*sizeRemoveIcon 默认是gone 这里获取不到宽度*/
        selectedTab?.let {
            val startWidthValue = it.root.measuredWidth
            val endWidthValue = it.root.editTextSearch.measuredWidth + sizeRemoveIcon + it.root.paddingLeft+it.root.paddingRight
    ...
    private fun removeTab(viewItemBinding: ViewItemBinding) {
        ...
        when {
            ...
            removeIndex == currentChildCount - 1 -> {
                ...
                selectTab(newSelectedViewBinding!!,viewItemBinding)
                binding.layoutItemContainer.removeView(viewItemBinding.root)
    ...
    private fun selectTab(viewSelectTab: ViewItemBinding,viewDeleteTab : ViewItemBinding? = null) {
        val indicatorCurrentXPosition = binding.viewIndicator.x
        val indicatorTargetXPosition = if(viewDeleteTab == null) viewSelectTab.root.x + viewSelectTab.editTextSearch.width / 2 - binding.viewIndicator.width/2
                                               else viewDeleteTab.root.x+ viewSelectTab.editTextSearch.width / 2 -binding.viewIndicator.width/2
        ...
        viewSelectTab.imageViewRemove.visibility = View.VISIBLE
        viewSelectTab.editTextSearch.alpha = 1f
    }
